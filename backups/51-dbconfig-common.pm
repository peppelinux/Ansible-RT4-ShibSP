...
Set($DatabaseAdminPassword , 'rt_secret');

Set($DatabaseUser , 'rt_user');
Set($DatabasePassword , 'rt_secret');

# SQLite needs a special case, since $DatabaseName must be a full pathname
my $dbc_dbname = 'rt4db'; if ( "mysql" eq "sqlite3" ) { Set ($DatabaseName, '' . '/' . $dbc_dbname); } else { Set ($DatabaseName, $dbc_dbname); }
